...
```{r}
mykNN2 <- function(train, test, y_train, y_test, k = 3, weighted = TRUE) {
  
  yhat <- vector(mode = "numeric", length = length(train))

  dist <- dist(train, method = "euclidean")
  ...
  # Returning named list
  return(dist)#invisible(list(yhat = yhat, residuals = residuals, SSE = SSE, k = k)))
...
train <- Auto_new[1:ntrain,5]
test <- Auto_new[(ntrain+1):nrow(Auto_new),5]
y_train <- Auto_new[1:ntrain,1]
y_test <- Auto_new[(ntrain+1):nrow(Auto_new),1]

train <- as.data.frame(train)
#test <- as.data.frame(test)
trial <- mykNN(train, test, y_train, y_test, k = 5)
trial
